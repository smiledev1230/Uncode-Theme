...
if (!class_exists('unheader')) {
	class unheader
	{
		...
		function __construct($args, $page_title = '', $subheading = '')
		{
			...
			if (isset($args['_uncode_menu_no_padding'])) $header_no_padding = $args['_uncode_menu_no_padding'][0];
			if (isset($args['_uncode_menu_no_padding_mobile'])) $header_no_padding_mobile = $args['_uncode_menu_no_padding_mobile'][0];
			...
			if ($header_no_padding_mobile === 'on') $header_no_padding .= 'remove-menu-padding-mobile ';
			...
			switch ($header_type)
			{
				...
				case 'header_basic':
					...
					$header_kburns = (isset($args['_uncode_header_kburns'][0]) && $args['_uncode_header_kburns'][0] == 'on') ? ' with-kburns' : '';
			...
			case 'header_uncodeblock':
				...
				foreach ($matches as $key => $value) {
					...
					if (isset($matches_attr[0]) && $matches_attr[0]!=='') {
						$replacement = '[vc_custom_heading is_header="yes" ' . $value[1] . $subheading . ']' . $page_title . '[/vc_custom_heading]';
